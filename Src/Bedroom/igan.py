...
def produce(img_cont, mask_pix, mask_lat):
    ...
    with tf.variable_scope('Paraphrasing') as scope:
        pb = tf.get_variable('Paint_board', [N_SAMPLES, 128], initializer=tf.random_uniform_initializer(minval=-1, maxval=1))
    ...
    # Loss
    loss_cont, loss_col = igan_loss(sess, net_cont, net_vgg, img_cont, mask_pix)
