...
def parse_args():
  ...
  parser.add_argument('--device', type=str, 
    default='/gpu:0',
    choices=['/gpu:0', '/cpu:0', '/gpu:1', '/gpu:2', '/gpu:3'],
  ...
  if args.video:
    ...
    path = os.path.join(args.img_output_dir,args.content_layers[0])
    maybe_make_directory(path)
    args.img_output_dir = path
  return args
...
def build_model(input_img, is_gen=False, model_dir = 'Model/CelebA/vgg-19.mat', mask=None):
  ...
  if is_gen:
    ...
    n, h, w, d     = input_img.shape
    net['input']   = tf.Variable(np.zeros((n, h, w, d), dtype=np.float32))
  if mask is not None:
    net['input_masked'] = net['input']*mask
    #if args.verbose: print('LAYER GROUP 1')
    #net['conv1_1'] = conv_layer('conv1_1', net['input_masked'], W=get_weights(vgg_layers, 0))
  #else:
...
def my_gram_matrix(x, area, depth):
  #mean  = tf.expand_dims(tf.reduce_mean(x, [3]), [3])
  #x = x - mean
  x = tf.transpose(x, [0,3,1,2])
  F = tf.reshape(x, (depth, area))
  ...
  G = tf.matmul(tf.transpose(F-F_mean),F-F_mean)
  return G
...
'''
#from models import *
def stylize(content_img, init_img, init_z=None):
  #global args
  #args = parse_args()
  ...
  with tf.device(args.device), tf.Session(config=config) as sess:
    ...
    minimize_with_adam(sess, net, optimizer, init, L_total)
    #if args.optimizer == 'adam':
    #  minimize_with_adam(sess, net, optimizer, init, L_total)
    #elif args.optimizer == 'lbfgs':
    #  minimize_with_lbfgs(sess, net, optimizer, init)
    
    output_img = sess.run(net['input'])
    
    output_img = convert_to_original_colors(np.copy(content_img), output_img)

    if init.get_shape().ndims<3:
        output_img = sess.run(init_img)
...
def get_optimizer(loss, optimizer='adam'):
...
def get_content_image(img_dir, max_size):
  path = img_dir
  # bgr image
  img = cv2.imread(path, cv2.IMREAD_COLOR)
  check_image(img, path)
  ...
  print(img.shape)
  img = cv2.resize(img, dsize=(mx,mx), interpolation=cv2.INTER_AREA)
  #img = preprocess(img)
  return img
...
def render_single_image(content_img, init_z, Generator):
  ...
  with tf.Graph().as_default():
    ...
    tick = time.time()
    stylize(content_img, style_imgs, init_z, Generator)
    tock = time.time()
...
def main():
  ...
  if args.video: render_video()
    ...
    render_single_image()
    #from mark_rf import marker
    #img_dir = os.path.join(args.img_output_dir, args.img_name)
    #out_img = os.path.join(img_dir, 'result_rf.png')
    #target_layer = args.content_layers[0]
    #marker(img_dir, out_img, target_layer)
