...
class GUI(QMainWindow):
    ...
    def initResources(self):
        ...
        self.is_first = True
        self.backpix = np.zeros((320, 320, 4), np.uint8)
    ...
    def initUI(self):
        ...
        self.setWindowTitle('PreservingGAN - Bedroom')
    ...
    def pressDrawBtn(self):
        ...
        x, y, h, w = (self.imgLbl.x, self.imgLbl.y, self.imgLbl.h, self.imgLbl.w)

        for i in range(max(x, 0), min(x+h, 320)):
            for j in range(max(y, 0), min(y+w, 320)):
                self.backpix[i, j, :] = dat[i, j, :]
    ...
    def pressGenBtn(self):
        ...
        img_cont = cv2.resize(self.backpix, (64, 64))
        mask_pix = np.zeros((320, 320), np.float32)

        for reg in self.regions:
            x, y, h, w = reg

            for i in range(x, min(x+h,320)):
                for j in range(y, min(y+w,320)):
                    if i>=0 and i<320 and y>=0 and y<320:
                        mask_pix[i][j] = 1

        mask_pix = cv2.resize(mask_pix, (64, 64), interpolation=cv2.INTER_AREA)
        mask_lat = cv2.resize(mask_pix, (8, 8), interpolation=cv2.INTER_AREA)
